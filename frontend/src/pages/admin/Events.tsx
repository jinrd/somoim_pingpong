...
const getStatusClassName = (status: EventStatus): string => {
  switch (status) {
    case "draft":
...
export default function Events() {
  ...
  const handleEventCreated = (createdEvent: SomoimEvent) => {
    setEvents((currentEvents) => {
      return [...currentEvents, createdEvent].sort(compareEvents);
    });
  };
    ...
    <section className={styles.page}>
      <div className={styles.pageHeader}>
        <div>
          ...
          <p className={styles.pageDescription}>
            모임 회차를 만들고 참석자와 경기 진행 상태를 관리합니다.
          </p>
        </div>
        ...
        >
          <Plus size={18} aria-hidden="true" />새 회차 만들기
        </button>
      </div>
      ...
      {isLoading ? (
        ...
        <div className={styles.emptyPanel}>
          ...
          >
            <Plus size={18} aria-hidden="true" />첫 회차 만들기
          </button>
        </div>
      ) : (
              ...
              >
                <div className={styles.eventCardContent}>
                  ...
                  <p className={styles.eventNotice}>
                    {event.notice?.trim() || "등록된 공지사항이 없습니다."}
                  </p>
                </div>
