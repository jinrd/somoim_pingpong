...
export default function Members() {
  ...
  const loadData = async () => {
    setError('');
    try {
      const [membersData, settingsData] = await Promise.all([getMembers(), getRankSettings()]);
      setMembers(membersData);
      setRankSettings(settingsData);
    } catch {
      setError('회원 정보를 불러오지 못했습니다. 서버 연결과 로그인 상태를 확인해 주세요.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className={styles.container}>
      <div className={styles.header}>
        ...
        <div className={styles.actions}>
          <button onClick={() => setRankModalOpen(true)} className={styles.btnSecondary}>
            <Settings size={18} style={{ verticalAlign: 'middle', marginRight: 4 }} /> 
            부수 승강 기준 설정
          </button>
          <button onClick={openNewMember} className={styles.btnPrimary}>
            <UserPlus size={18} style={{ verticalAlign: 'middle', marginRight: 4 }} />
            새 회원 추가
          </button>
        </div>
      </div>
        ...
        <table className={styles.table}>
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr><td colSpan={7} className={styles.emptyCell}>등록된 회원이 없습니다.</td></tr>
            ) : (
              members.map(member => (
                <tr key={member.id}>
                  <td>{member.name}</td>
                  <td>{member.nickname}</td>
                  <td><strong>{member.rank}부</strong></td>
                  <td>{member.gender === 'M' ? '남성' : member.gender === 'F' ? '여성' : '-'}</td>
                  <td>{member.phone || '-'}</td>
                  <td>
                    <span className={member.status === 'active' ? styles.activeBadge : styles.inactiveBadge}>
                      {member.status === 'active' ? '활동중' : '비활동'}
                    </span>
                  </td>
                  <td>
                    <button onClick={() => openEditMember(member)} className={styles.iconButton} aria-label={`${member.nickname} 회원 수정`}>
                      <Edit size={18} />
                    </button>
                  </td>
                </tr>
