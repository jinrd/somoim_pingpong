...
export const getRankSettings = async (): Promise<RankSettings | null> => {
  const records = await pb.collection('rank_settings').getList<RankSettings>(1, 1, {
    sort: '-updated',
  });
...
// 승강 기준 수정 (또는 생성)
export const saveRankSettings = async (id: string | null, data: RankSettingsInput): Promise<RankSettings> => {
  if (id) {
