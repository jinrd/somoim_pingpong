  ...
  (db) => {
    ...
    if (records.length === 0) {
      ...
      for (const record of records) {
        ...
        const fallbackRank = Math.min(Math.max(8, minRank), maxRank);
    ...
    const memberDefaultField = rankSettings.schema.getFieldByName(
      "default_member_rank",
    );
    const guestDefaultField =
      rankSettings.schema.getFieldByName("default_guest_rank");
